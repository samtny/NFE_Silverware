...
// WILL DISABLE PROGRAMMING AFTER GYRO CALIBRATION - 2 - 3 seconds after
//powerup)
...
void USART1_IRQHandler(void)
{
  ...
  // USART_ReceiveData inline
  rx_buffer[rx_end] = (uint16_t)(USART1->RDR & (uint16_t)0x01FF);
  // USART_ReceiveData inline end
  ...
  // USART_GetFlagStatus inline
  if ((USART1->ISR & USART_FLAG_ORE) != (uint16_t)RESET)
  // USART_GetFlagStatus inline end
  {
    ...
    // USART_ClearFlag inline
    USART1->ICR = USART_FLAG_ORE;
    // USART_ClearFlag inline end
...
void checkrx()
{
  ...
  if (frame_received)
  {
    ...
    if (rx_state == 1)
    {
      ...
      // requires 10 good frames to come in before rx_ready safety can be
      //toggled to 1
      if (bind_safety > 9)
      {

        // because aux channels initialize low and clear the binding while armed
        //flag before aux updates high
        rx_ready = 1;
        bind_safety = 10;
      }
    }
